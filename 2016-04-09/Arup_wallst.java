...
public class Arup_wallst {
	...
	public Arup_wallst(Scanner fin) {
	...
	public int runMCM() {
		...
		for (int c = 1; c < minMult.length; c++) {
			...
			for (int o = 0; o < minMult.length-c; o++) {
				...
				System.out.printf("length: %d, depth: %d, cost: %d\n", min.length, min.depth, min.cost);
...
//		Scanner fin = new Scanner(new File("wallst.in"));
		...
		for (int my_case=1; my_case<=numcases; my_case++) {
			...
			Arup_wallst myBuildings = new Arup_wallst(fin);
